...
const Page = () => {
  ...
  async function fetchSingleProduct(){
    try {
        const res = await axios.get(`/api/mobiles/${id}`)
        setData(res.data)
    } catch (error) {
        console.log(error)
    }
  ...
  const { name, company, price, image,camara,storage } = data;
  return (
    <AppLayout>
      <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl md:w-[80%] md:h-[500px] w-full">
        <div class="md:flex">
          ...
          <div class="p-8">
            ...
            <p class="mt-2 text-gray-600"> {camara}</p>
            <p class="mt-2 text-gray-600">Price: ₹{price}</p>
            <span class="mt-2 text-white px-5 rounded-r-2xl bg-blue-700">{storage}</span>
